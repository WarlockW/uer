...
#### 文本生成（语言模型）
我们可以使用 *generate_lm.py* 来用语言模型生成文本。给定文本开头，模型根据开头续写。使用 *generate.py* 加载[GPT-2-distil](https://share.weiyun.com/IAvDbjKR)模型并进行生成示例：
```
...
```
*english.txt* 包含了要翻译的内容。*generated_sentence.txt* 包含了翻译后的内容。*--load_model_path* 可以使用经过seq2seq目标预先训练的模型。
