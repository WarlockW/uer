...
#### 文本生成（Seq2seq模型）
我们可以使用 *generate_seq2seq.py* 来用Seq2seq模型进行文本生成。例如英译中模型[iwslt_en_zh](https://share.weiyun.com/QOfBQ4u4)，给定英文，模型根据英文翻译出中文。使用 *generate_seq2seq.py* 加载英译中模型并进行生成示例：
```
python3 scripts/generate_seq2seq.py --load_model_path models/iwslt_en_zh_model.bin-50000 --vocab_path models/google_uncased_en_vocab.txt \
                                    ...
                                    --encoder transformer --decoder transformer --target seq2seq
```
