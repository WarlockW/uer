UER支持多种下游任务，包括文本分类（classification）、文本对分类（pair classification）、基于文档的问答（document-based question answering）、序列标注（sequence labeling）、机器阅读理解（machine reading comprehension）等。注意下游任务使用的词向量层和编码器层需要与预训练模型相匹配，配置文件也需要保持一致。
