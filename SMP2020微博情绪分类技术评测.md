### SMP2020-EWECT 常规任务
我们随机选择了一些预训练模型并对其微调，进行K折交叉验证，对训练数据集做出预测，之后生成特征（预测概率）以进行堆叠：
```
...
这是使用堆叠模型的简单演示，以上操作可以为我们带来非常有竞争力的结果（可以拿到前5名的成绩）。当我们继续添加具有不同预处理，预训练和微调策略的模型时，将会获得更多的改进。 可以在[比赛首页]（http://39.97.118.137/）上找到更多详细信息。