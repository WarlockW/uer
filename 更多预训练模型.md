...
*--embedding word* 表示使用传统的词向量，LSTM不需要位置向量。此外，layernorm在传统RNN模型中出现的不多。因此我们可以加上 *--remove_embedding_layernorm* 。不过，这里我们加上layernorm也没有关系。
...
```
T5使用的语料的格式和GPT一样。我们可以通过 *--relative_position_embedding* 指定使用相对位置编码；通过 *--remove_embedding_layernorm_bias* 和 *--remove_transformer_bias* 指定模型的一些部分去掉bias；通过 *--feed_forward* 指定feed-forward层的类型。由于T5使用编码器-解码器结构，因此我们需要同时指定 *--encoder* 和 *--decoder* 。


#### 更多组合
使用LSTM编码器和LM目标任务预训练示例：
```
...
使用机器翻译语料和任务进行预训练示例，思路和CoVe一样，只不过使用Transformer作为编码器：
```
...
python3 pretrain.py --dataset_path dataset.pt --vocab_path models/google_zh_vocab.txt --tgt_vocab_path models/google_uncased_en_vocab.txt \
                    ...
                    --target seq2seq --learning_rate 1e-4 \
                    --report_steps 1000 --total_steps 50000 --save_checkpoint_steps 10000
```
其中 [*iwslt_15_zh_en.tsv*](https://share.weiyun.com/hIKCDpf6) 为中英平行语料。我们可以使用预训练模型的编码器部分去初始化下游任务模型。
<br>