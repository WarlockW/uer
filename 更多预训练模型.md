...
```
T5使用的语料的格式和GPT一样。我们通过 *--relative_position_embedding* 指定使用相对位置编码；通过 *--remove_embedding_layernorm* 和 *--layernorm_positioning pre* 指定使用前置layernorm（和GPT-2一样）。由于T5使用编码器-解码器结构，因此我们需要同时指定 *--encoder* 和 *--decoder* 。
