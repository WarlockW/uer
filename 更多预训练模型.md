...
```
T5使用的语料的格式和GPT一样。我们通过 *--relative_position_embedding* 指定使用相对位置编码；通过 *--remove_embedding_layernorm_bias* 和 *--remove_transformer_bias* 指定模型的一些部分去掉bias；通过 *--feed_forward* 指定feed-forward层的类型。由于T5使用编码器-解码器结构，因此我们需要同时指定 *--encoder* 和 *--decoder* 。
